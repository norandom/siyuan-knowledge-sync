...
func TestMove_HappyPath_PreservesIdentityFields(t *testing.T) {
	...
	if err := tr.Move("a.md", "Sysadmin & DevOps/a.md"); err != nil {
	...
	got, ok := tr.Get("Sysadmin & DevOps/a.md")
	if !ok {
		t.Fatal("expected new key 'Sysadmin & DevOps/a.md' to be present after Move")
	}
	...
	if got.LocalPath != "Sysadmin & DevOps/a.md" {
		t.Errorf("LocalPath not updated: got %q want %q", got.LocalPath, "Sysadmin & DevOps/a.md")
	}
}
...
func TestMove_PersistenceRoundTrip(t *testing.T) {
	...
	got, ok := reloaded.Get("Sysadmin & DevOps/a.md")
	if !ok {
		t.Fatal("reloaded state missing new key 'Sysadmin & DevOps/a.md'")
	}
